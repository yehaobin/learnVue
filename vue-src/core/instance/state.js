...
function initData (vm: Component) {
  ...
  while (i--) {
    ...
    } else if (!isReserved(key)) { // data中不要使用$和_开头，因为在Vue内部也使用$和_作为方法或属性，这是为了防止冲突。
      proxy(vm, `_data`, key) // 好处: 更方便的操作 data 中的数据
    }
  }
  // observe data
...
function initComputed (vm: Component, computed: Object) {
  ...
  for (const key in computed) {
    ...
    // 计算属性可能是一个function,也可以是包含get和set的对象
    ...
    if (!isSSR) {
      ...
      watchers[key] = new Watcher(
        vm,
        getter || noop, // 与render watcher不同，这里传入的用户自定义的get方法
        noop,
        computedWatcherOptions // lazy为true, 初始化时不需要调用watcher的get方法
      )
    }
...
function createComputedGetter (key) {
  return function computedGetter () {
    ...
    if (watcher) {
      ...
      if (Dep.target) {
        // 给当前watcher实例依赖的deps添加render watcher
        watcher.depend()
      }
